...
//		state="asue2_i44";
		...
		mc_base._xscale=Math.floor(100*640/800);
		mc_base._yscale=mc_base._xscale;
		talky_setup();
	...
	var pmc;
		...
		if(up.state=="menu") { state="intro_title"; } // force menu
...
//dbg.print(file_lines[0]);
		...
		for(i=0;i<mcs_max;i++)
		{
			...
			if(!preload)
			{
				...
				if(lin[2]=="inv") // goes in overlay layer
				{
					mcs[i]=gfx.add_clip(omc,file_name,null,-400,-300);
				}
				else // goes in main layer
				{
				...
				if(lin[0]=="esc_2_write_1")
				{
					mcs[i].tf=gfx.create_text_html(mcs[i],null,365,435,100,100);
					gfx.set_text_html(mcs[i].tf,50,0xffff00,"<b>00</b>");
				}
			}
		...
		mc_score.tf1=gfx.create_text_html(mc_score,null,200,0,200,75);
...
// scroll room over		
	...
	var pmc;
		...
		{
			if(state_split[0]!="intro")
			{
				if(saves.gameover==null)
				{
					score--;
				}
			}
		}
		if(state_split[0]=="intro") // hide score?
		{
...
var talky_state;
	...
	{
		talky_mc=gfx.create_clip(up.mc,null,300,100);
//		talky_mc.onRelease=delegate(click,"talky",-1);
	...
	function talky_display(str)
	{
		...
		gfx.set_text_html(talky_tf,32,0xffffff,"<b>"+str+"</b>");
		
		gfx.clear(talky_mc);
		talky_mc.style.out=0xc0000000;
		talky_mc.style.fill=0xc0000000;
