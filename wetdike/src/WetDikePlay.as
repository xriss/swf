...
//					_root.wetdike.state_next=WetDike.STATE_PLAY;
			...
			f=function()
			{
				if(!_root.popup)
				{
					this.final_scores((_root.wetdike.game_seed+1)&0xffff);
//					_root.wetdike.game_seed=(_root.wetdike.game_seed+1)&0xffff;
//					_root.wetdike.state_next=WetDike.STATE_PLAY;
				}
			};
...
//					_root.wetdike.game_seed=Math.floor(Math.random()*65536)&0xffff;
//					_root.wetdike.state_next=WetDike.STATE_PLAY;
				}
			};
...
//		dbg.print(seed); //13366
		...
		_root.signals.signal("wetdike","set",this);
...
// build comms defaults
		...
		_root.signals.signal("wetdike","start",this);
	}
	
	function clean()
	{
		_root.signals.signal("wetdike","end",this);
...
//		up.dikescores.clean();
	...
	{
		_root.signals.signal("wetdike","high",this);
		up.dikescores.setup();
	}
...
//		up.dikescores.update();
		...
		_root.signals.signal("wetdike","update",this);
		
		if(exit_after_scores)
		{
			if(_root.popup==null)
			{
				_root.wetdike.game_seed=next_seed;
				_root.wetdike.state_next=WetDike.STATE_PLAY;
			}
		}
		...
		{
			_root.wetdike.game_seed=_root.wetdike.next_game_seed;
			_root.wetdike.state_next=WetDike.STATE_PLAY;
			_root.wetdike.next_game_seed=null;
		}
	}
