...
// stack in hand
		...
		stacks[13].mc.filters = [ new flash.filters.DropShadowFilter(5, 45, 0x000000, 1, 10, 10, 2, 3) ];
...
// check where we can place the floating cards
	...
	var t,tx,ty;
		...
		for(i=0;i<13;i++)
		{
			...
			if(i==0)
			{
				if( (stacks[13].cards.length==1) && (from_stack==1) ) // allow a single undeal of one card
				{
					s.place_legal=true;
				}
				else
			...
			{
				if( (stacks[13].cards.length>0) && (s.place_test(stacks[13])==stacks[13].cards.length) )
				{
					s.place_legal=true;
				}
				else
			...
			if(i==0) // really weight away from special costly undeal 1card move
			{
				s.place_pos=((fx-tx)*(fx-tx))+((fy-ty)*(fy-ty));
				s.place_pos=s.place_pos*s.place_pos*s.place_pos/512;
			}
			else
...
// fadeout ghosts
		...
		if(stacks[14].fadeout==true)
		{
			stacks[14].mc._alpha*=0.75;
			if(stacks[14].mc._alpha<1)
			{
				stacks[14].fadeout=false;
				stacks[14].clear_cards();
				stacks[14].mc._alpha=50;
			}
		}
		else
		{
...
//					dbg.print(idx+" MOVE DEAL "+s);
				...
				else
				{
					...
					stacks[14].fadeout=true;//clear_cards();
...
// reset all animations in progress
			...
			if(from_stack==-1)
			{
				...
				if( (x>s.mc._x-50) && (x<s.mc._x+50) && (y>s.mc._y-71) && (y<s.mc._y+71) )
				{
					from_stack=0;
					hand_x=x-(s.mc._x); 
					hand_y=y-(s.mc._y);
			...
			if(from_stack==-1)
			{
				...
				if(top)
				{
					if( (x>s.mc._x+top.x-50) && (x<s.mc._x+top.x+50) && (y>s.mc._y+top.y-71) && (y<s.mc._y+top.y+71) )
					{
						s1=stacks[13];
						s2=stacks[14];
						from_stack=1;
						hand_x=x-(s.mc._x+top.x); 
						hand_y=y-(s.mc._y+top.y);
						s.deal(1,s1,false);
						s1.spread_layout();
						s1.clone(s2);
						s2.fadeout=false;
			...
			for(i=2;i<13;i++)
			{
				...
				{
					s=stacks[i];
					for(j=s.cards.length-1;j>=s.cards.length-s.pickupable;j--)
					{
						top=s.cards[j];
						if(top)
						{
...
// deal cards
			...
			if(stacks[13].cards.length>0) // 1 or more cards in hand
			{				
				check_placements();
										
				if( (to_stack==from_stack) && (stacks[13].cards.length==1) ) // a click
				{
...
// record click and click time for next double click test
					...
					n=stacks[13].cards.length;
					for(i=0;i<n;i++) // animate
					{
						add_animcard();
						set_animcard_from(1,stacks[13].cards[i]);
					}
					
					stacks[13].deal(stacks[13].cards.length,stacks[to_stack]);
					stacks[14].fadeout=true;//clear_cards();
					...
					{
						if( animcards[animcards.length-1].get_dist2()>50*50 )
						{
							sfx_slide();
						}
					}
			...
			else
			{
				...
				if((lstclk_stack==-1)&&((frame_time-lstclk_time)<15)) // double click background to auto put away
				{
					...
					for(i=1 ; (i<13)&&(to_stack<0) ; i++ )
					{
						if((i>1) && (i<6)) { continue; }				
						if(stacks[i].cards.length<1) { continue; }
						if(stacks[i].cards[stacks[i].cards.length-1].back==true) { continue; }
						
						stacks[i].deal(1,stacks[13]);

// try to put away
						if(stacks[2].place_test(stacks[13])==stacks[13].cards.length) { to_stack=2; }
						if(stacks[3].place_test(stacks[13])==stacks[13].cards.length) { to_stack=3; }
						if(stacks[4].place_test(stacks[13])==stacks[13].cards.length) { to_stack=4; }
						if(stacks[5].place_test(stacks[13])==stacks[13].cards.length) { to_stack=5; }
					
						if(to_stack!=-1) // found a card to move
						{
							from_stack=i;
						
							stacks[13].deal(1,stacks[from_stack]);
...
//					dbg.print(movenum+" MOVE "+from_stack+" -> "+to_stack+s);
		...
		if(stacks[13].cards.length>0) // cards in hand
		{
			check_placements();
			
//			stacks[to_stack].spread_layout_force();
			
			stacks[14].mc._x=stacks[to_stack].place_x+stacks[to_stack].mc._x;
			stacks[14].mc._y=stacks[to_stack].place_y+stacks[to_stack].mc._y;
...
// check end game after animations finish
		...
		if(animcards.length==0)
		{
				...
				)
			{
				...
				{
					_root.signals.signal("wetdike","won",this);
