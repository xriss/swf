 ...
 *
 * @author Simon Wacker
 * @see org.as2lib.env.log.logger.AudiofarmLogger
 */
class org.as2lib.env.log.repository.AudiofarmLoggerRepository extends BasicClass implements LoggerRepository {
	
	/** Already received loggers. */
	private var loggers:Object;
	
	/**
	 * Constructs a new {@code AudiofarmLoggerRepository} instance.
	 */
	...
	public function getLogger(name:String):Logger {
		...
		var logger:Logger = new AudiofarmLogger(name);
		loggers[name] = logger;
		return logger;
	}
