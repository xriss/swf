...
class org.as2lib.env.log.logger.DebugItLogger extends AbstractLogger implements Logger {
	 ...
	 * the DebugIt Component from Robert Hoekman.
	 * 
	 * <p>You can use this method as trace method for MTASC's trace support:
	 * <code>mtasc ... -trace org.as2lib.env.log.logger.DebugItLogger.trace</code>
	 * 
	...
	public static function trace(message, location:String, fileName:String, lineNumber:Number):Void {
		Debug.write((new MtascLogMessage(message, location, fileName, lineNumber)).toString());
	}
	...
	public function log(message, level:LogLevel):Void {
		if (isEnabled(level)) {
			Debug.write((new SimpleLogMessage(message, level, name)).toString());
		}
	}
