...
//3d origin	and bounds
		...
		bmc=gfx.add_clip(mc,"level_01_bak",null,0,600-level.ph);
		bmc.cacheAsBitmap=true;
...
// process collision data
		...
		tards[0].px=400;//start_x;
		tards[0].py=-300;//start_y;
...
//		fmc.bmp=gfx.add_clip(fmc,level.img_for,null,0,0);
		...
		tfd=gfx.create_text_html(mco,null,0,0,800,600);
		txd=[];
...
//		tards[1].talk=talky.create(tards[1].mc,0,-30);
		...
		_root.wetplay.PlaySFX("sfx_001",3,65536,1); playsfx="001";

#else

		_root.wetplay.PlaySFX("sfx_004",3,65536,1); playsfx="004";
...
var wiblur;
	...
	var i;
		...
		if(mute==2)
		{
			_root.wetplay.PlaySFX(null,3,65535,0.50);
		}
		else
		if(mute==3)
		{
		...
		if(score>(4096*4))
		{
			if(playsfx!="005")
			{
				_root.wetplay.PlaySFX("sfx_005",3,65536,1); playsfx="005";
			}
		}
		else
		if(score>(4096*3))
		...
		if(score>(4096*2))
		{
			if(playsfx!="002")
			{
				_root.wetplay.PlaySFX("sfx_002",3,65536,1); playsfx="002";
			}
		}
		else
		if(score>(4096*1))
		{
			if(playsfx!="003")
			{
				_root.wetplay.PlaySFX("sfx_003",3,65536,1); playsfx="003";
			}
		}
#end		
...
//		var ny=focus.mc._y;
		...
		mc._x=-(focus.mc._x-400);
		mc._y=-(focus.mc._y-300);
		
		if(mc._x>x2_min) 		{mc._x=x2_min;}
		if(mc._y>(y2_max-600)) 	{mc._y=(y2_max-600);}

		if(mc._x<-(x2_max-800))	{mc._x=-(x2_max-800);}
		if(mc._y<y2_min)		{mc._y=y2_min;}
...
//					dbg.print(x+" "+y);
				...
				{
//					dbg.print(x+" "+y);
					pickups[pickups.length]=new PlayItem(this,x,y,"mute_m");
				}
				else
				if(ids[ 0]==0xf1)
				{
//					dbg.print(x+" "+y);
					pickups[pickups.length]=new PlayItem(this,x,y,"mute_u");
				}
				else
				if(ids[ 0]==0xf2)
				{
//					dbg.print(x+" "+y);
					pickups[pickups.length]=new PlayItem(this,x,y,"mute_t");
				}
				else
				if(ids[ 0]==0xf3)
				{
//					dbg.print(x+" "+y);
					pickups[pickups.length]=new PlayItem(this,x,y,"mute_e");
				}
				else
				if(ids[ 0]==253)
				{
...
//					dbg.print(x+" "+y);
	...
	function getwet(x,y)
	{
		...
		return water[( x*#(WCELL))+(y*level.cw*#(WCELL))+1 ];
	}
