...
class Vbrain_pot
{
	...
	function Vbrain_pot(_up)
	{
		up=_up;
		up.type_name="vobj_pot"
	}
...
//		gfx.draw_box(up.mc.hitArea,0,-20,-80,40,80);
		...
		up.nodes["2"].balloon=new Vballoon(up.up);
		up.nodes["3"].balloon=new Vballoon(up.up);
		
		up.nodes["1"].balloon.owner = up;
		up.nodes["2"].balloon.owner = up;
		up.nodes["3"].balloon.owner = up;


		up.nodes["1"].balloon.node = up.nodes["1"] ;
		up.nodes["2"].balloon.node = up.nodes["2"] ;
		up.nodes["3"].balloon.node = up.nodes["3"] ;
		
		up.nodes["1"].balloon.setup();
		up.nodes["2"].balloon.setup();
		up.nodes["3"].balloon.setup();
		
//		up.nodes["1"].balloon.set_style( "items1" , 16 , 5 , 5 );
//		up.nodes["2"].balloon.set_style( "items1" , 16 , 5 , 5 );
//		up.nodes["3"].balloon.set_style( "items1" , 16 , 5 , 5 );
	...
	function clean()
	{
		...
		up.nodes["2"].balloon.clean();
		up.nodes["3"].balloon.clean();
	}
...
//dbg.print(props.rot)
	...
	function update()
	{
		...
		up.nodes["2"].balloon.update();
		up.nodes["3"].balloon.update();
