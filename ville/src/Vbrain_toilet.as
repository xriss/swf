...
class Vbrain_toilet
{
	...
	function Vbrain_toilet(_up)
	{
		up=_up;
		up.type_name="vobj_toilet"
	}
...
//dbg.print(d+":"+e.nodeType+":"+e.nodeName);
	...
	function do_use(cc)
	{
		...
		cc.vcobj.send_msg_prop( "xyz" , (up.px+40)+":"+up.py+":"+(up.pz+40) );
	...
	var aa;
		...
		if(props.dirt) // new dirt setting
		{
			...
			if(dirt>4) {dirt=4;}
			if(dirt<0) {dirt=0;}
			up.anim.page_base="open_dirt"+dirt;
		}
	...
	function update()
	{
		...
		if( (up.focus.vcobj.props.focus==up.vcobj.id) && (up.vcobj.props.focus==up.focus.vcobj.id) ) // someone is using us
		{
			if	(
					(up.focus.pz==up.pz+40) &&
					(up.focus.px==up.px+40)
				)
			{
				up.anim.page_base="shut_dirt"+dirt;
