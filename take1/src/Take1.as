...
//		lv_wonderful.sendAndLoad('http://swf.wetgenes.com/swf/wonderful.php?id=3489',lv_wonderful,"POST");
	...
	var date=new Date();
		...
		over=new OverField(this); over.setup();
...
// go no further until we are loged in, loaded and setup
	...
	if(state_next!=null)
		...
		if(state)
		{
			update_time=Math.floor(update_time*3/4); // add some decay to stored time so we can over run a little bit without it building up
