...
// blend between these two numbers, precise mouse position is only guaranteed on sampled frames
	...
	function setup()
	{
		...
		keymap.fire[0]=Key.SPACE;
		keymap.fire[1]=Key.CONTROL;
		
		keymap.left[0]=Key.LEFT;
		keymap.left[1]=65;
		
		keymap.right[0]=Key.RIGHT;
		keymap.right[1]=68;
		
		keymap.up[0]=Key.UP;
		keymap.up[1]=87;
		
		keymap.down[0]=Key.DOWN;
		keymap.down[1]=83;
...
// scan ahead in the string and find the next mouse pos
	...
	function apply_key_on_to_prekey(c,k)
	{
		if( (c==keymap.fire[0]) || (c==keymap.fire[1]) )
		{
			prekey_on(KEY_FIRE);
		}
		else
		if( (c==keymap.up[0]) || (c==keymap.up[1]) )
		{
			prekey_on(KEY_UP);
		}
		else
		if( (c==keymap.down[0]) || (c==keymap.down[1]) )
		{
			prekey_on(KEY_DOWN);
		}
		else
		if( (c==keymap.left[0]) || (c==keymap.left[1]) )
		{
			prekey_on(KEY_LEFT);
		}
		else
		if( (c==keymap.right[0]) || (c==keymap.right[1]) )
		{
			prekey_on(KEY_RIGHT);
		}
	}
	
	function apply_key_off_to_prekey(c,k)
	{
//dbg.print(c+" : "+k);
		if( (c==keymap.fire[0]) || (c==keymap.fire[1]) )
		{
	...
	{
		if( (Key.isDown(keymap.fire[0])) || (Key.isDown(keymap.fire[1])) )
		{
			prekey_on(KEY_FIRE);
		}
		else
		...
		if( (Key.isDown(keymap.up[0])) || (Key.isDown(keymap.up[1])) )
		{
			prekey_on(KEY_UP);
		}
		else
		...
		if( (Key.isDown(keymap.down[0])) || (Key.isDown(keymap.down[1])) )
		{
			prekey_on(KEY_DOWN);
		}
		else
		...
		if( (Key.isDown(keymap.left[0])) || (Key.isDown(keymap.left[1])) )
		{
			prekey_on(KEY_LEFT);
		}
		else
		...
		if( (Key.isDown(keymap.right[0])) || (Key.isDown(keymap.right[1])) )
		{
			prekey_on(KEY_RIGHT);
		}
		else
