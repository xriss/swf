...
// ser
	...
	var it;
		...
		var names=["unlocked","extreme","ultimate"];
		var name;
		for(i=0;i<3;i++)
		{
			name=names[i];
			_root.bmc.remember( "pixl_txt_"+name , bmcache.create_img , //this is safe as it only works once
			{
				url:"pixl_txt_"+name ,
			...
			bms[name]=_root.bmc.getbmp("pixl_txt_"+name);
		}
...
//		bmidx=bmnxt;
	...
	var r1=new flash.geom.Rectangle(0, 0, 64, 16);
		...
		bms[bmidx].copyPixels(bms[s], r1, p1,null,null,true);
	}
...
//		dbg.print(x+","+y);
		...
		bms[bmidx].fillRect( new flash.geom.Rectangle(xp-wp, yp-hp, wp*2, hp*2) , c );
	}
