...
//		gfx.set_text_html(mcs.high.txt,24,0xffffffff,"<b><p align=\"center\">High</p></b>");		
		...
		if(up.up.lselect.save[up.up.game_seed].map)
		{
			str_to_map( up.up.lselect.save[up.up.game_seed].map );
		}
...
//		if(up.level)
		{
			up.up.lselect.save[up.up.game_seed].map=map_to_str();
		}
		
		up.up.lselect.so_save(); // force save
	...
	{
	var i;
		rnd_seed( up.up.game_seed ); // make sure we have the right seed
		...
		if( c && ( tim_mod < c.born ) ) // do not allow juggling, and only count new when a creep gets in
		{
			if(oldscore<score)
			{
				...
				if(up.up.game_seed<25) // save total score in mochi or other peoples bucket
				{
					...
					_root.signals.signal("rgbtd0","final",up);
				}
			}
			
			up.score=score; // save score where the signals can find it
			up.replay=map_to_str();
			up.up.lselect.save[up.up.game_seed].map=up.replay; // remember map layout for later
			
			_root.signals.signal("rgbtd0","high",up);
		}
